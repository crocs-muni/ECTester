...
/**
 * An example class which provides ECC utility functions for setting and creating EC keypairs.
 * This class also contains the domain parameters of the SECG prime and binary field curves.
 *
...
public class ECExample {
    ...
    public static KeyPair generatKeyPair(short keyLength, short keyClass, byte[] buffer, short offset) {
        ...
        short length = 0;
        if (keyClass == KeyPair.ALG_EC_FP) {
            length = getCurveParameter(curve, PARAMETER_FP, buffer, offset);
            priv.setFieldFP(buffer, offset, length);
            ...
            if (length == 2) {
                ...
                short i1 = Util.getShort(buffer, offset);
                short i2 = Util.getShort(buffer, (short) offset + 2);
                short i3 = Util.getShort(buffer, (short) offset + 4);
                priv.setFieldF2M(i3, i2, i1);
                pub.setFieldF2M(i3, i2, i1);
            }
        }
        length = getCurveParameter(curve, PARAMETER_A, buffer, offset);
        priv.setA(buffer, offset, length);
        pub.setA(buffer, offset, length);

        length = getCurveParameter(curve, PARAMETER_B, buffer, offset);
        priv.setB(buffer, offset, length);
        pub.setB(buffer, offset, length);

        length = getCurveParameter(curve, PARAMETER_G, buffer, offset);
        priv.setG(buffer, offset, length);
        pub.setG(buffer, offset, length);

        length = getCurveParameter(curve, PARAMETER_R, buffer, offset);
        priv.setR(buffer, offset, length);
        pub.setR(buffer, offset, length);

        getCurveParameter(curve, PARAMETER_K, buffer, offset);
        short k = Util.getShort(buffer, offset);
        priv.setK(k);
        pub.setK(k);
